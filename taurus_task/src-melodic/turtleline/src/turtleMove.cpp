...
struct Myturtle
{
    ...
    float y;
    float theta;
}turtle_present_pose,turtle_target_pose,turtle_origin_pose;
...
void execute(const turtleline::turtleMoveGoalConstPtr& goal, Server* as)
{
    ...
    {  
        ...
        if(!isTurnComple)
        {
            vel_msgs.angular.z = 10 * (atan2(turtle_target_pose.y-turtle_present_pose.y,
        ...
        if(isTurnComple)
        {
            // vel_msgs.linear.x = PID_realize(1.5 * sqrt(pow(turtle_target_pose.x-turtle_present_pose.x, 2) +
            //     pow(turtle_target_pose.y-turtle_present_pose.y, 2)));
            vel_msgs.linear.x = 5 * sqrt(pow(turtle_target_pose.x-turtle_present_pose.x, 2) +
                pow(turtle_target_pose.y-turtle_present_pose.y, 2));
        ...
        ROS_INFO("vel_msgs.angular.z: %f",vel_msgs.angular.z);
