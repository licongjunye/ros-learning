...
QNode::QNode(int argc, char** argv) : init_argc(argc), init_argv(argv) {
  //    读取topic的设置
  QSettings topic_setting("taurusnavqt", "settings");
  ...
  QSettings settings("taurusnavqt", "Displays");
...
bool QNode::init() {
  ros::init(init_argc, init_argv, "taurusnavqt",
...
bool QNode::init(const std::string& master_url, const std::string& host_url) {
  ...
  ros::init(remappings, "taurusnavqt", ros::init_options::AnonymousName);
  if (!ros::master::check()) {
    return false;
  }
  ros::start();  // explicitly needed since our nodehandle is going out of
...
void QNode::SubAndPubTopic() {
  ...
  //机器人状态发布
  robotstatus_pub = n.advertise<roborts_msgs::RobotStatus>("/Robot_Status",100);
  //全局规划Path
  m_plannerPathSub =
...
void QNode::slot_robotstatus(float keyBoard, float tgz_X, float tgz_Y)
{
  roborts_msgs::RobotStatus rs;
  // rs.header.frame_id = 'frame';
  ...
  // rs.blue_y = 0;
  // rs.bullet_freq = 0;
  // rs.bullet_remaining_num = 0;
  // rs.bullet_speed = 0;
  // rs.game_progress = 0;
  // rs.game_type = 0;
  // rs.hurt_type = 0;
  rs.key_board = keyBoard;
  // rs.max_HP = 0;
  // rs.other_robot_id = 0;
  // rs.red_confiden = 0;
  // rs.red_x = 0;
  ...
  // rs.remain_HP = 0;
  // rs.robot_id = 0;
  // rs.shooter_cooling_limit = 0;
  // rs.shooter_cooling_rate = 0;
  // rs.shooter_speed_limit = 0;
  rs.tgx = tgz_X;
  rs.tgy = tgz_Y;
  // rs.tgz = 0;
  qDebug()<<"keyboard:"<<rs.key_board<<"tgz_X"<<rs.tgx<<"tgz_Y"<<rs.tgy<<endl;
  robotstatus_pub.publish(rs);
}
